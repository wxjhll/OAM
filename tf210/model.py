...
def unet(input_shape=(128,128,1), num_classes=1):
    ...
    # 解码器
    up6 = tf.keras.layers.Conv2DTranspose(256, kernel_size=(2, 2), strides=(2, 2), padding="same")(conv4)
    concat6 = tf.keras.layers.concatenate([up6, conv3], axis=-1)
    conv6 = conv_block(concat6, 256, kernel_size=3)

    up7 = tf.keras.layers.Conv2DTranspose(128, kernel_size=(2, 2), strides=(2, 2), padding="same")(conv6)
    concat7 = tf.keras.layers.concatenate([up7, conv2], axis=-1)
    conv7 = conv_block(concat7, 128, kernel_size=3)

    up8 = tf.keras.layers.Conv2DTranspose(64, kernel_size=(2, 2), strides=(2, 2), padding="same")(conv7)
    concat8 = tf.keras.layers.concatenate([up8, conv1], axis=-1)
    conv8 = conv_block(concat8, 64, kernel_size=3)
